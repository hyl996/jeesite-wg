...
public class OfficeController extends BaseController {
	...
	public String save(@Validated Office office, String cmd, Model model) {
		if (StringUtils.isBlank(office.getParentCode())) {
			office.getExtend().setExtendS1(AbsEnumType.isJt);
		}else {
			office.getExtend().setExtendS1(AbsEnumType.isZz);
		}
	...
	public String delete(Office office) {
		...
		//删除方法改写 BY LY2019-11-26
		baseDao.deleteSql("delete from js_sys_user_data_scope where ctrl_data='"+office.getOfficeCode()+"'");//删除数据权限
		baseDao.deleteSql("delete from js_sys_office where office_code='"+office.getOfficeCode()+"'");//删除自身
	...
	public List<Map<String, Object>> treeData(String excludeCode, String parentCode, Boolean isAll,
			...
			Boolean isLoadUser, String postCode, String roleCode, String ctrlPermi,String isZz,String isJt) {
		...
		if (isZz!=null) {
			...
			extend.setExtendS1(AbsEnumType.isZz);
			where.setExtend(extend);
		}
		if (isJt!=null) {
			Extend extend = new Extend();
			extend.setExtendS1(AbsEnumType.isJt);
			where.setExtend(extend);
		}
