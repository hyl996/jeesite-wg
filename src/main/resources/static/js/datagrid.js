(function($) {
	var DataGrid = function(options, $this) {
		var dataGrid = typeof $this != "undefined" ? $this
				: options.dataGrid ? options.dataGrid : $("#dataGrid"), dataGridId = dataGrid
				.attr("id"), dataGridPage = options.dataGridPage ? options.dataGridPage
				: $("#" + dataGridId + "Page"), searchForm = options.searchForm ? options.searchForm
				: $("#searchForm"), options = $
				.extend(
						{
									...
									dataGrid
											.jqGrid(
													"addRow",
													{
														...
														addRowParams : {
															keys : true,
															focusField : action == "new"
														},
														initdata : typeof options.editGridAddRowInitData == "function" ? options
																.editGridAddRowInitData(dataGridId)
							...
							beforeRequest : function(data) {
								...
								case "script":
									if (options.url == undefined
											|| options.url == "") {
										js
												.showMessage("设置searchForm表单指定错误或URL为空。");
										return false
									}
								}
								js.loading();
										...
										if (postData.nodeid) {
											if (options.expandNodeClearPostData == true) {
												...
												if (typeof options.expandNodeClearPostData == "string") {
													$
															.each(
																	options.expandNodeClearPostData
																			.split(","),
																	function(
																			idx,
																			val) {
							...
							loadComplete : function(data) {
								...
								if (options.editGrid) {
									$(function() {
										...
										if (ids.length > 0) {
											...
											if (options.editGridInitRowNum
													&& options.editGridAddRow) {
								...
								if (typeof options.btnEventBind == "function") {
									options.btnEventBind(dataGrid
											.find(".btnList"));
									options.btnEventBind($(
											"#" + dataGridId + "_frozen").find(
											".btnList"))
								}
								if (typeof options.btnMoreEventBind == "function") {
									options.btnMoreEventBind(dataGrid
											.find(".btnMore"));
									options.btnMoreEventBind($(
											"#" + dataGridId + "_frozen").find(
											".btnList"))
								}
							...
							onSortCol : function(index, iCol, sortorder) {
								if (options.inputOrderBy
										&& options.inputOrderBy.length) {
									options.inputOrderBy.val(index + " "
											+ sortorder)
								}
							},
																				...
																				.confirm(
																						...
																						function(
																							...
																							if (data.result == Global.TRUE) {
																								var confirmSuccess = se
																										.data("confirmSuccess");
																								if (confirmSuccess != undefined) {
																									try {
																										eval(confirmSuccess)
																									} catch (e) {
																										log("confirmSuccess error: "
																												+ e)
																									}
																								} else {
																									if (options.treeGrid) {
																										...
																										if (delTreeNode != undefined) {
																											...
																											var row = getRowData(se
																													.parents(
																															".jqgrow")
																													.attr(
																															"id"));
																											if (row
																													&& !isRoot(row.parentCode)) {
							...
							btnMoreEventBind : function(elements) {
								elements
										.each(function() {
											if ($(this).attr(
													"data-mouseover-binded") == undefined) {
												$(this)
														.attr(
																"data-mouseover-binded",
																true);
												var timeoutHover;
												elements
														.mouseover(function() {
															var bdiv = dataGrid
																	.closest(".ui-jqgrid-bdiv"), more = $(this), items = more
																	.next(), pos = more
															...
															items
																	...
																	.show();
															if (items
																	.attr("data-hover-binded") == undefined) {
																items
																		.attr(
																				"data-hover-binded",
																				true);
		...
		if (options.inputPageNo.length == 0) {
			...
			searchForm
					.append('<input id="pageNo" name="pageNo" type="hidden" value="'
							+ (pageNo ? pageNo : "") + '"/>');
		...
		if (options.inputPageSize.length == 0) {
			...
			searchForm
					.append('<input id="pageSize" name="pageSize" type="hidden" value="'
							+ (pageSize ? pageSize : "") + '"/>');
		...
		if (options.inputOrderBy.length == 0) {
			...
			searchForm
					.append('<input id="orderBy" name="orderBy" type="hidden" value="'
							+ (orderBy ? orderBy : "") + '"/>');
								...
								setTimeout(
										function() {
											if ($.fn.iCheck !== undefined) {
												searchForm
														.find(
																"input[type=checkbox].form-control:not(.noicheck),input[type=radio].form-control:not(.noicheck)")
														.iCheck("update")
											}
											if ($.fn.select2 !== undefined) {
												searchForm
														.find(
																"select.form-control:not(.noselect2)")
														.trigger("change")
											}
		...
		if (options.btnSearch.length > 0) {
			options.btnSearch.unbind("click").click(
					function() {
						var btnSearch = $(this);
						if (searchForm.hasClass("hide")) {
							...
							btnSearch.html(btnSearch.html().replace("查询", "隐藏")
									.replace($.jgrid.extend.btnSearch,
											$.jgrid.extend.btnHideSearch))
						} else {
							searchForm.addClass("hide");
							btnSearch.removeClass("active");
							btnSearch.html(btnSearch.html().replace("隐藏", "查询")
									.replace($.jgrid.extend.btnHideSearch,
											$.jgrid.extend.btnSearch))
						}
						resizeDataGrid();
						return false
					});
			if (!searchForm.hasClass("hide")) {
				...
				btnSearch.addClass("active");
				btnSearch
						.html(btnSearch.html().replace("查询", "隐藏").replace(
								$.jgrid.extend.btnSearch,
								$.jgrid.extend.btnHideSearch))
			}
		}
		...
		if (options.editGrid) {
			...
			if (options.editGridInputForm
					&& options.editGridInputForm.length > 0
					&& options.editGridInputFormListName != ""
					&& options.editGridInputFormListAttrs != "") {
				options.editGridInputForm.submit(function() {
					updateListFieldName(options.editGridInputFormListName,
							options.editGridInputFormListAttrs)
				})
			}
		}
		...
		function setGridHeight() {
			...
			if (typeof options.autoGridHeight == "function") {
				...
				if (tabContent.length > 0) {
					var portlet = $(dataGrid).parents(
							".box.tabbable.autoHeight:not(.default)");
					if (portlet.length > 0) {
						var headerHeight = $(dataGrid).parents(".ui-jqgrid")
								.find(".ui-jqgrid-hdiv").height();
						if (headerHeight == 0) {
							headerHeight = 43;
							if (options.groupHeaders
									&& options.groupHeaders.twoLevel) {
								if (options.groupHeaders.threeLevel) {
									headerHeight *= 3
								} else {
									headerHeight *= 2
						...
						gridHeight = tabContent.height() - headerHeight
								- options.autoGridHeightFix
					} else {
						gridHeight = 300
					}
				} else {
					...
					gridHeight = ($(window).height() - $("body").height()
							+ gridHeight - options.autoGridHeightFix);
					if (gridHeight < 200) {
						gridHeight = 200
					}
		...
		function setGridWidth() {
			...
			if (gridWidth != 0) {
				dataGrid.jqGrid("setGridWidth", gridWidth
						- options.autoGridWidthFix, (options.shrinkToFit && $(
						window).width() > 500))
			}
		}
		...
		function expandTreeNode(level) {
			...
			setTimeout(function() {
				...
				if (dataGrid.expandNodeIds.length > 100) {
					js.showMessage($.jgrid.format($.jgrid.extend.expandTooMany,
							dataGrid.expandNodeIds.length));
					dataGrid.expandNodeIds = []
				} else {
					$(
							"#" + dataGrid.expandNodeIds.shift()
									+ ":visible .tree-plus", dataGrid).click()
				}
		...
		function mergeCell(cellNames) {
			...
			$.each(cellNames.split(","),
					function(idx, name) {
						var bg = trs.eq(0).children(
								"[aria-describedby='" + dataGridId + "_" + name
										+ "']"), index = bg.index(), rowsp = 1;
		...
		function updateListFieldName(listName, fieldNames) {
			$.each(fieldNames.split(","), function(idx, fieldName) {
				if (fieldName != "") {
					...
					dataGrid.find(inputs.join(",")).each(
							function(idx, val) {
								$(this)
										.attr(
												"name",
					...
					var defaultInputs = [
							...
							'[name^="!' + listName + '["][name$="[].'
									+ fieldName + '"]:not(div,a,script)' ];
					dataGrid.find(defaultInputs.join(",")).each(
							function(idx, val) {
								$(this).attr(
										"name",
										"!" + listName + "[" + idx + "]."
		...
		var $set = this.each(function() {
			...
			if (typeof option === "string"
				...
				if (value instanceof Array) {
					method_call = data[option].apply(data, value, v2, v3, v4,
							v5)
				} else {
...
function pageHtml(c) {
	...
	if (c.pageNo == c.first) {
		b
				.push('<li class="disabled"><a href="javascript:"><i class="fa fa-angle-left"></i></a></li>\n')
	} else {
		b.push('<li><a href="javascript:" onclick="' + c.funcName + "("
				+ c.prev + "," + c.pageSize + ",'" + c.funcParam
				+ '\');"> <i class="fa fa-angle-left"></i></a></li>\n')
	}
	for (a = c.first; a < c.first + c.bothNum && a < c.centerBegin; a++) {
		b.push('<li><a href="javascript:" onclick="' + c.funcName + "(" + a
				+ "," + c.pageSize + ",'" + c.funcParam + "');\">"
				+ (a + 1 - c.first) + "</a></li>\n")
	}
	if (a < c.centerBegin) {
	...
	for (a = c.centerBegin; a <= c.centerEnd; a++) {
		if (a == c.pageNo) {
			b.push('<li class="active"><a href="javascript:">'
					+ (a + 1 - c.first) + "</a></li>\n")
		} else {
			b.push('<li><a href="javascript:" onclick="' + c.funcName + "(" + a
					+ "," + c.pageSize + ",'" + c.funcParam + "');\">"
	...
	if (c.pageNo == c.last) {
		b
				.push('<li class="disabled"><a href="javascript:"><i class="fa fa-angle-right"></i></a></li>\n')
	} else {
		b.push('<li><a href="javascript:" onclick="' + c.funcName + "("
				+ c.next + "," + c.pageSize + ",'" + c.funcParam
...
function pageHtmlControl(b) {
	...
	a.push('<li class="disabled controls" title="' + $.jgrid.extend.pageTitle
			+ '">' + $.jgrid.extend.pageLabelA + " ");
	a
			.push('<input type="text" value="'
					+ b.pageNo
					+ '" onkeypress="var e=window.event||event;var c=e.keyCode||e.which;if(c==13)');
	a.push(b.funcName + "(this.value," + b.pageSize + ",'" + b.funcParam
			+ '\');" onclick="this.select();"/> ' + $.jgrid.extend.pageLabelB
			+ " ");
	a
			.push('<input type="text" value="'
					+ b.pageSize
					+ '" onkeypress="var e=window.event||event;var c=e.keyCode||e.which;if(c==13)');
	a.push(b.funcName + "(" + b.pageNo + ",this.value,'" + b.funcParam
			+ '\');" onclick="this.select();"/> ');
	...
	a.jgrid
			.extend({
				getIdsByLevel : function(b) {
					...
					this
							.each(function() {
								...
								var l = g.p.expColInd, k = g.p.treeReader.expanded_field, e = g.p.treeReader.leaf_field, d = g.p.treeReader.level_field, m = g.p.treeReader.icon_field, j = g.p.treeReader.loaded;
								var h = g.p.data;
				...
				delTreeChildNode : function(b) {
					return this
							.each(function() {
								var h = this, q = h.p.localReader.id, j, g = h.p.treeReader.left_field, p = h.p.treeReader.right_field, l = h.p.treeReader.expanded_field, k = h.p.treeReader.loaded, d, e, m, o;
	...
	a.jgrid
			.extend({
				setComplexHeaders : function(b) {
					...
					return this
							.each(function() {
								...
								var K = this, p = l.defaultStyle === undefined ? true
										: l.defaultStyle, G, E, n, J, k, d, F, x, M, u, c, C, r = 0, H = 0, A = false, B, v, w, f, y = K.p.colModel, D = y.length, t = K.grid.headers, h = a(
										"table.ui-jqgrid-htable", K.grid.hDiv), m = h
										.children("thead").children(
												"tr.ui-jqgrid-labels:last")
										.addClass("jqg-second-row-header"), e = h
										.children("thead"), s, q, o = h
								...
								J = a("<tr>", {
									role : "rowheader"
										...
										"ui-jqgrid-labels jqg-four-row-header");
								for (G = 0; G < D; G++) {
									d = t[G].el;
									F = a(d);
									E = y[G];
								...
								a(K).bind("jqGridResizeStop.setGroupHeaders",
										function(O, N, i) {
				...
				createFrozenHtable : function(e, g) {
					var P = this[0], I = P.p.complexHeader, m = I.complexHeaders, q = m.defaultStyle, k = m.threeLevel, j = m.twoLevel, H = P.p.id, A = P.p.colModel, F = e + 1, t = P.grid.headers, r = 0, L = 0, C = false, K, G, z, d, J, y, B, R, l, D, u, b, E, v, w, h, c = P.grid.fhDiv
							.height(), M = a("<table class='ui-jqgrid-htable' style='width:1px;height:"
							+ c
							+ "px' role='grid' aria-labelledby='gbox_"
							+ H
							+ "' cellspacing='0' cellpadding='0' border='0'></table>"), f = a("<thead></thead>"), p = a(
							"<tr>", {
								role : "row",
								"aria-hidden" : "true"
							...
							"auto"), n = a("<tr>", {
						role : "rowheader"
					}).addClass("ui-jqgrid-labels jqg-second-row-header"), o = a(
							"<tr>", {
								role : "rowheader"
							})
							.addClass("ui-jqgrid-labels jqg-third-row-header"), O = a(
							"<tr>", {
								role : "rowheader"
							}).addClass("ui-jqgrid-labels jqg-four-row-header"), N = function(
							V, T) {
						var S = 0, U = T.length;
						for (; S < U; S++) {
					...
					for (K = 0; K < F; K++) {
						...
						z = {
							height : "0px",
							width : t[K].width + "px",
							display : (G.hidden ? "none" : "")
						};
						a("<th>", {
							role : "gridcell"
						}).css(z).addClass("ui-first-th-" + P.p.direction)
								.appendTo(p);
						b = N(G.name, k);
						if (b >= 0) {
							D = k[b];
