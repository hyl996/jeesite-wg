...
$("#btnRefHt").click(function (){
    ...
    js.layer.open({
    	...
    	shade: 0.2,
    	maxmin: true,
    	content: '/js/a/wght/wgContract/reflist',
    	btn: ['确认', '取消'],
    	yes: function(index, layero){
    		...
    		js.addTabPage($(this), "新增合同修订", "/js/a/wght/wgContract/refToAdd?pk="+rowId, null, null);//弹出新的标签页
    	},
	...
	$.ajax({
		...
		success: function (msg) {
			if(msg!=null&&msg!=""){
				 ...
				 */
				js.addTabPage($(this), "修改合同管理", "/js/a/wght/wgContract/formXD?pkContract="+rowIds[0]+"&isEdit=true", null, null);//弹出新的标签页
			}
		}
	});
});
...
$('#btnApproveAll2').click(function(){
	...
	for(var i=0;i<selectRows.length;i++){
		...
		if(dataselect.vbillstatus=="1"||dataselect.vbillstatus=="审批通过"){
...
$('#btnUnApproveAll2').click(function(){
	...
	for(var i=0;i<selectRows.length;i++){
		var dataselect=$('#dataGrid').getRowData(selectRows[i]);//获取行数据
		if(dataselect.vbillstatus=="0"||dataselect.vbillstatus==""||dataselect.vbillstatus=="自由"){
			js.showMessage('单据已取消审批，不可再次取消审批！','','info',5000);
			return ;
		}
		selectData.push(dataselect.id);
	}
