...
$(document).ready(function() {
	...
	$('#ctInvoiceapplySkmxDataGridAddRowBtn').click(function (){
		$(".input-group").on("mousedown",function showDeptTreeRef(){
			var id=$(this).attr("id");
			oldRefName=$(this).context.children[0].value;
			oldRefCode=$(this).context.children[1].value;
	...
	$('#ctInvoiceapplyKpxxDataGridAddRowBtn').click(function (){
		$(".input-group").on("mousedown",function showDeptTreeRef(){
			var id=$(this).attr("id");
			oldRefName=$(this).context.children[0].value;
			oldRefCode=$(this).context.children[1].value;
...
function treeselectCallback(id, act, index, layero, nodes){
	...
	if(id.startsWith("ysproj_") && (act == 'ok' || act == 'clear')){//收费项目
		var rowid=id.split("_")[1];
		if(nodes!=null&&nodes!=undefined){
			var newpro=$("#"+id+"Code").val();
			if(oldRefCode==newpro){
		...
		$("#kpcostp_"+rowid+"_kpcostpCode").val($("#"+id+"Code").val());
		$("#kpcostp_"+rowid+"_kpcostpName").val($("#"+id+"Name").val());
	}
}
...
function listselectCallback(id, act, index, layero,selectData,nodes){
	...
	if(id.startsWith('cust_') && (act == 'ok' || act == 'clear')){
		...
		//第一行客户修改更新开票信息
		var arrs=$("#ctInvoiceapplySkmxDataGrid").getDataIDs();
		if(rowid!=arrs[0]){
			return ;
		}
		if(selectData!=null&&selectData!=undefined&&selectData[newvue]!=undefined){
			...
			$.ajax({
				...
				success: function (data) {
					//清空信息
					var arrs=$("#ctInvoiceapplyKpxxDataGrid").getDataIDs();
					for(let i=0;i<arrs.length;i++){
						$("#ctInvoiceapplyKpxxDataGrid").dataGrid('delRowData',arrs[i]);
					}
					...
					if(dlist!=null&&dlist.length>0){
						...
						$("#ctInvoiceapplyKpxxDataGrid").jqGrid('addRowData', rowid, context, 'last');
						$('#ctInvoiceapplyKpxxDataGrid').jqGrid('editRow', rowid, true);
					}else{
					}
				},
				error: function(e){
...
$('.box-main').on('change','.nsqkpmny' ,function(){
	...
	if(flag=="true"){
		...
		if(mny-sykp>0){
			js.showMessage('申请开票金额不能大于剩余可开票金额!');
			$('#'+rowId+"_nsqkpmny").val(null);
			return ;
		}
	}
	//计算税额和无税金额
	...
	var hjmny=0.00;
	for(let i=0;i<arrs.length;i++){
		var sqkpmny=$("#"+arrs[i]+"_nsqkpmny").val();
		hjmny=Number(hjmny)+Number(sqkpmny);
	}
	$('#nsqmny').val(fixnum(hjmny));
	form_jine();//金额格式化
});
